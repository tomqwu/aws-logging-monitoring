terraform {
  ...
  backend "atlas" {
    name    = "CPPIB/edl-centralized-logging-prod"
...
data "terraform_remote_state" "edl-etl-workflow" {
  ...
  config {
    name    = "CPPIB/edl-etl-workflow-prod"
    address = "https://terraform.cppib.ca"
  }
}
...
data "terraform_remote_state" "elasticsearch" {
  ...
  config {
    name    = "CPPIB/edl-elasticsearch-prod"
    address = "https://terraform.cppib.ca"
  }
}

# cppib.io dns zone is hosted on shared-services-dev account
